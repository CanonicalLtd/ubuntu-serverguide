...
!!! Note: If you attempt to create an LVM physical volume on a whole device on which
...
!!! Note: Every time either `/etc/lvm.conf` or `/etc/multipath.conf` is updated, the
...
!!! Note: To work around a quirk in multipathd, when an `/etc/multipath.conf` doesn't
...
1.  Determine which disks are the internal disks and mark them as the ones
...
!!! Note: Whenever a hardware\_handler is specified, it is your responsibility to
...
!!! Note: Whenever `multipath.conf` is updated, so should the initrd by executing
...
!!! Note: The default **verbosity** level of multipath is **2** and can be globally
